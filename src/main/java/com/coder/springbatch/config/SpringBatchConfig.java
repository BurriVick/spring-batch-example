package com.coder.springbatch.config;
...
public class SpringBatchConfig {
	...
	public Job job(JobBuilderFactory jobBuilderFactory, StepBuilderFactory stepBuilderFactory,
		...
		Step step = stepBuilderFactory.get("STEP").<User, User>chunk(100).reader(itemReader)
				.processor(itemProcessor).writer(itemWriter).build();

		Job job = jobBuilderFactory.get("JOB").incrementer(new RunIdIncrementer()).start(step).build();
	...
	public LineMapper<User> lineMapper() {
		...
		lineTokenizer.setNames(new String[] { "id", "name", "age", "salary" });
